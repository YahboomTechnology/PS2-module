...
PSS_RX = 5    #Right rocker X axis data
PSS_RY = 6    #Right rocker Y axis data
PSS_LX = 7    #Left rocker X axis data
PSS_LY = 8    #Right rocker Y axis data

#Define the running state of the car
enSTOP = 0
enRUN =1
enBACK = 2
enLEFT = 3
...
#Definition of pin of car motor
IN1 = 20
IN2 = 21
IN3 = 19
IN4 = 26
...
def init():
    ...
    pwm_ENA.start(0)
    pwm_ENB.start(0)
    #Set the frequency and starting duty ratio of the steering gear
...
#The car moves along the left front 
def upleft():
    GPIO.output(IN1, GPIO.HIGH)
    GPIO.output(IN2, GPIO.LOW)
    GPIO.output(IN3, GPIO.HIGH)
...
#The car moves along the left rear 
def downleft():
    GPIO.output(IN1, GPIO.LOW)
    GPIO.output(IN2, GPIO.HIGH)
    GPIO.output(IN3, GPIO.LOW)
...
#The car moves along the upper right
def upright():
    GPIO.output(IN1, GPIO.HIGH)
    GPIO.output(IN2, GPIO.LOW)
    GPIO.output(IN3, GPIO.HIGH)
...
#The car moves along the lower right
def downright():
    GPIO.output(IN1, GPIO.LOW)
    GPIO.output(IN2, GPIO.HIGH)
    GPIO.output(IN3, GPIO.LOW)
...
#The servo rotates to a specified angle
...
try:
	...
	if g_Carstate == enSTOP:
	    ...
	    brake()
 #Necessary delays to avoid sending handle commands too frequently		 
	time.sleep(0.5)
